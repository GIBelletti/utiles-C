...
////
bool abb_in_order_nodo_desde_hasta(nodo_t* nodo, bool visitar(const char*, void*, void*), void* extra, const char* desde, const char* hasta, abb_comparar_clave_t comparar){
	if(nodo == NULL)
		return true;
///////
	int comparacion_desde = comparar(nodo->clave, desde);
	int comparacion_hasta = comparar(nodo->clave, hasta);
	if(comparacion_desde < 0){
		return abb_in_order_nodo_desde_hasta(nodo->derecha, visitar, extra, desde, hasta, comparar);
	}
	if (comparacion_hasta <= 0){
		if (!abb_in_order_nodo_desde_hasta(nodo->izquierda, visitar, extra, desde, hasta, comparar)){
			return false;
		}
		if(!visitar(nodo->clave, nodo->valor, extra)){
			return false;
		}
		if(!abb_in_order_nodo_desde_hasta(nodo->derecha, visitar, extra, desde, hasta, comparar)){
			return false;
		}
	}
	if (comparacion_hasta > 0){
		return abb_in_order_nodo_desde_hasta(nodo->izquierda, visitar, extra, desde, hasta, comparar);
	}
/////////
	return true;
}
...
void abb_in_order_desde_hasta(abb_t *arbol, bool visitar(const char *, void *, void *), void *extra, const char* desde, const char* hasta){
	abb_in_order_nodo_desde_hasta(arbol->raiz, visitar, extra, desde, hasta, arbol->comparar);
	return;
}
